  ...
  app.get("/ai/executive-summary", auth, async function(req, res) {
    try {
      ...
      if (!openai) {
        return res.json({
          source: "fallback",
          summary: fallbackSummary + " Add OPENAI_API_KEY in server/.env to enable GPT-generated executive advice."
        });
      }
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are NovaOps AI, a concise business operations advisor. Give a practical executive summary for a small business owner. Keep it under 120 words. Mention urgent risks first, then next best actions."
          },
          {
            role: "user",
      ...
      const summary = completion.choices[0]?.message?.content || fallbackSummary;
