...
const API = "http://192.168.4.27:4000";
...
export default function App() {
  ...
  async function checkStripeReturn() {
    try {
      ...
      if (data.user) {
        ...
        const savedToken = localStorage.getItem("novaops_token");
        if (savedToken) {
          setToken(savedToken);
          localStorage.setItem("novaops_user", JSON.stringify(data.user));
  ...
  async function restoreSavedLogin() {
    try {
      ...
      const savedToken = localStorage.getItem("novaops_token");
      const savedUser = localStorage.getItem("novaops_user");
  ...
  function signOut() {
    try {
      if (typeof window !== "undefined") {
        ...
        localStorage.removeItem("novaops_user");
      }
  ...
  async function signup() {
    try {
      ...
      setToken(data.token);
      setUser(data.user);
      localStorage.setItem("novaops_token", data.token);
      localStorage.setItem("novaops_user", JSON.stringify(data.user));
